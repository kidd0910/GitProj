...
'Vers		Date       	By		Notes
'--------------	--------------	--------------	----------------------------------
'0.0.1		2017/12/06	Tim   		Copy Create
'----------------------------------------------------------------------------------
...
NameSpace File.Business
    ''' <summary>
    ''' CtAttchFileByCase 管理附加檔案CT For Case_No
    ''' </summary>
    Partial Public Class CtAttchFileByCase
...
#Region "CASE_SEQ 補正次數, REF. APPL020.CASE_SEQ"
        '' <summary>
        '' CASE_SEQ 補正次數, REF. APPL020.CASE_SEQ
        '' </summary>
...
#Region "MoveFile 搬移至附加檔"
        ...
        Public Sub MoveFile()
            Try
                ...
                entTemp.SqlRetrictions = "PAGE_NAME = '" & Me.PAGE_NAME & "' AND " &
...
#Region "MoveFileToTemp 搬移檔案至暫存檔"
        ...
        Public Sub MoveFileToTemp()
            Try
                ...
                '=== 取得要搬移的資料清單 ===
                Dim cond As StringBuilder = New StringBuilder()
                cond.Append("ACCE_SOURCE = '" & Me.PAGE_NAME & "' AND " &
...
#Region "GetAllFile 取得所有檔案"
        ...
        Public Function GetAllFile() As DataTable
            Try
                ...
                Me.ProcessQueryCondition(condition, "=", "CASE_NO", Me.CASE_NO) '案號
